...
{
    public abstract class SolutionWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static SolutionWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(SolutionWebMvcModule).Assembly.FullName);
        }
        ...
        {
            using (var context = IocManager.Resolve<SolutionDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected async Task UsingDbContextAsync(Func<SolutionDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<SolutionDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<SolutionDbContext, Task<T>> func)
        {
            T result;
